...
if(!class_exists('Aelia_WP_Requirements_Checker')) {
	/**
	 * Checks that plugin's requirements are met. The class allows to check the
	 * PHP version, the presence of required PHP extensions and the presence of
	 * required plugins.
	 *
	 * This class has been exported from the original Aelia Foundation Classes,
	 * the shared framework used by Aelia to develop their plugins.
	 *
	 * @author Aelia (Diego Zanella) <admin@aelia.co>
	 * @version 1.5.1.150305
	 * @link http://aelia.co
	 */
	class Aelia_WP_Requirements_Checker {
		/**
		 ...
		 * @var string
		 */
		protected $text_domain = 'aelia-wp-requirements-check';
		 ...
		 * @var string
		 */
		protected $plugin_name = 'Some Plugin Name';
		 ...
		 *
		 * @var string
		 * @see Aelia_WP_Requirements_Checker::js_url
		 * @since 1.5.1.150305
		 */
		...
		/**
		 * An array of PHP extensions required by the plugin.
		 *
		 * @var array
		 */
		...
		protected $required_plugins = array(
			/* The example below illustrates how to add a plugin to the required ones.
			 * The key of each entry must be a plugin name (i.e. the exact name of a
			 * plugin, as entered in plugin metadata), while the values describe the
			 * requirements, as follows:
			 *
			 * - version: Required. The minimum plugin version required.
			 * - extra_info: Optional. A text message that will be displayed after the
			 *               "plugin X is missing". It can contain HTML.
			 * - url: Optional. If specified, it should point to a plugin file. When
			 *        the required plugin is missing, the class will allow to download
			 *        it automatically from this URL.
			 */
		 ...
		 *
		 * @return Aelia_WP_Requirements_Checker
		 */
		 ...
		 * for the frontend.
		 *
		 * IMPORTANT: this method relies on the assumption that the JavaScript file
		 * will be located in <plugin_folder>/src/js
		 *
		...
		public function plugin_requirements_notices() {
			...
			<div class="wp_aelia message error fade">
				<h4 class="wp_aeliamessage_header" style="margin: 1em 0 0 0"><?php
